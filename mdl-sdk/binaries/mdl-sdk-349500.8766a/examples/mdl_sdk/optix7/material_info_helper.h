...
// examples/mdl_sdk/optix7/material_info_helper.h
//
// Header file providing material parameter meta information for editing material parameters.
...
public:
    ...
    , m_device_arg_block(0)
    {
        ...
        if (arg_block != nullptr) {
            ...
            cudaMalloc(reinterpret_cast<void**>(&m_device_arg_block), m_arg_block->get_size());
            update_arg_block_on_gpu();
        }
    ...
    char *get_argument_block_data() const {
        if (!m_arg_block)
            return nullptr;
    ...
    size_t get_argument_block_size() const {
        if (!m_arg_block)
            return 0;
        return m_arg_block->get_size();
    }

    // Get the argument block on the GPU.
    char *get_argument_block_data_gpu() {
        return reinterpret_cast<char *>(m_device_arg_block);
    }

    // Updates the argument block on the GPU.
    void update_arg_block_on_gpu() {
        cudaMemcpy(
            reinterpret_cast<void*>(m_device_arg_block),
            m_arg_block->get_data(),
            m_arg_block->get_size(),
            cudaMemcpyHostToDevice);
    }

    // Updates the argument block on the GPU asynchronously.
    void update_arg_block_on_gpu(CUstream stream) {
        cudaMemcpyAsync(
            reinterpret_cast<void*>(m_device_arg_block),
            m_arg_block->get_data(),
            m_arg_block->get_size(),
    ...
    std::vector<std::string> const &get_referenced_scene_data_names() const {
        return m_referenced_scene_data_names;
    }
...
private:
    ...
    // device pointer of argument block
    CUdeviceptr m_device_arg_block;
    ...
    std::vector<std::string> m_referenced_scene_data_names;
};
